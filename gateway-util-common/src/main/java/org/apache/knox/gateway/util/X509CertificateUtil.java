...
public class X509CertificateUtil {
  ...
  public static X509Certificate generateCertificate(String dn, KeyPair pair, int days, String algorithm) {
    try {
      ...
      GeneralName[] sanNames = buildSanNames(hostname);
  ...
  private static GeneralName[] buildSanNames(String hostname) {
    try {
      ...
      if ("localhost".equals(hostname)) {
        ...
        if (Character.isAlphabetic(detectedHostname.charAt(0))) {
          names.add(new GeneralName(GeneralName.dNSName, detectedHostname));
        }
        ...
        if (Character.isAlphabetic(detectedFullyQualifiedHostname.charAt(0))) {
          names.add(new GeneralName(GeneralName.dNSName, detectedFullyQualifiedHostname));
        }
      }
